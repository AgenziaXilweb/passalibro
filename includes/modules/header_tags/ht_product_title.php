      ...
      if (basename($PHP_SELF) == FILENAME_PRODUCT_INFO) {
        if (isset($HTTP_GET_VARS['products_id'])) {
          if ($product_check['total'] > 0) {
            $product_info_query = tep_db_query("select p.products_model, pd.products_name, pd.products_description from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_status = '1' and p.products_id = '" . (int)$HTTP_GET_VARS['products_id'] . "' and pd.products_id = p.products_id and pd.language_id = '" . (int)$languages_id . "'");
            $product_info = tep_db_fetch_array($product_info_query);

            ### Marco - Modificato per miglioramento SEO

            $oscTemplate->setTitle($product_info['products_name'] . ', ' . $product_info['products_model'] . ', ' . $product_info['products_description'] );
