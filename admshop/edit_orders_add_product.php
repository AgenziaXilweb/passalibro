    ...
    // Add a product to the virtual cart
      case 'add_product':
        ...
        $sql_data_array = array('orders_id' => tep_db_prepare_input($oID),
                                ...
                                'products_price' => (float)tep_db_prepare_input($product['products_price']),
                                'products_used_price' => (float)tep_db_prepare_input($product['products_used_price']),
                                'final_price' => tep_db_prepare_input(($product['products_price'] + $AddedOptionsPrice)),
                                'final_used_price' => tep_db_prepare_input(($product['products_used_price'] + $AddedOptionsPrice)),
                                'products_tax' => tep_db_prepare_input($products_tax),
                                'products_quantity' => (int)tep_db_prepare_input($_POST['add_product_quantity']),
                                'products_quantity_reserved' => (int)tep_db_prepare_input($_POST['add_product_reserved_quantity']),
                                'products_used_quantity' => (int)tep_db_prepare_input($_POST['add_product_used_quantity']),
                                'sede_scuola' => $_SESSION['admsede'],
                                'sede_impegno_n' => (int)tep_db_prepare_input($_POST['sede_impegno_n']),
                                'sede_impegno_u' => (int)tep_db_prepare_input($_POST['sede_impegno_u']));
        tep_db_perform(TABLE_ORDERS_PRODUCTS, $sql_data_array);

//connect to database
    $connection = mysqli_connect(DB_SERVER, DB_SERVER_USERNAME, DB_SERVER_PASSWORD, DB_DATABASE,3306);      
//run the store proc
    $result_price = mysqli_query($connection, "CALL allinea_tot_man($oID)");
...
$quantita=isset($_POST['add_product_quantity'])?$_POST['add_product_quantity']:$_POST['add_product_used_quantity'];
			// Update products_ordered info
			tep_db_query ("UPDATE " . TABLE_PRODUCTS . " SET
			products_ordered = products_ordered + " . $quantita . "
			WHERE products_id = " . $_POST['add_product_products_id']);
...
// Generate product list based on chosen category or search keywords
  ...
  if (isset($_POST['search'])) {
    ...
    if (sizeof($search_array) <= 1) {
      $search_fields = array('p.products_id', 'p.products_price', 'p.products_used_price', 'p.products_model', 'pd.products_name');
    ...
    $products_query = tep_db_query("select p.products_id, p.products_price, p.products_used_price, p.products_model, pd.products_name from " . TABLE_PRODUCTS . " p left join " . TABLE_PRODUCTS_DESCRIPTION . " pd on (p.products_id = pd.products_id) where pd.language_id = '" . $languages_id . "' and (" . $product_search . ") order by pd.products_name");
...
<?php
  ...
  if ($step > 3) {
    echo '          <tr class="dataTableRow">' . "\n" .
         ...
         '            <td class="dataTableContent" align="left" valign="middle">' . TEXT_SCUOLA . '&nbsp;<input name="scuola" size="3" value="0">' . TEXT_CLASSE . '&nbsp;<input name="classe" size="3" value="0">' . TEXT_SEZIONE . '&nbsp;<input name="sezione" size="3" value="0"></td>' . "\n" .
         '            <td class="dataTableContent" align="center" valign="middle"></td>' . "\n" .
         '          </tr>' . "\n" .
         '          <tr class="dataTableRow">' . "\n" .
         '            <td colspan="3" style="border-top: 1px solid #C9C9C9;">' . TEXT_QUANTITY . '<input name="add_product_quantity" size="3" value="0"> Sede Impegno Nuovo:<input name="sede_impegno_n" size="3" value="0"></td>' . "\n" .
         '          </tr>' . "\n" .
         '          <tr class="dataTableRow">' . "\n" .
         '            <td colspan="3" style="border-top: 1px solid #C9C9C9;">' . TEXT_USED_QUANTITY . '<input name="add_product_used_quantity" size="3" value="0"> Sede Impegno Usato:<input name="sede_impegno_u" size="3" value="0"></td>' . "\n" .
         '          </tr>' . "\n" .
         '          <tr class="dataTableRow">' . "\n" .
         '            <<td colspan="3" style="border-top: 1px solid #C9C9C9;">' . TEXT_RESERVED_QUANTITY . '<input name="add_product_reserved_quantity" size="3" value="0"></td>' . "\n" .         
		 '          </tr>' . "\n" . 
		 '          <tr class="dataTableRow">' . "\n" .
		 '             <td></td>' . "\n" . 
