...
while ($sede_amazon = tep_db_fetch_array($sedi_amazon)) {
    ...
    $amazonListOrders = tep_db_query("SELECT amazonOrderId FROM " . TABLE_AMAZON_ORDER_LIST . " WHERE amazonSede = " . $sede_amazon['sede'] . " AND (amazonOrderStatus='Unshipped' OR amazonOrderStatus='Pending')");
    if ($totAmazonOrders=tep_db_num_rows($amazonListOrders)) {
        $orderIds = new MarketplaceWebServiceOrders_Model_OrderIdList();
        $arrayOrdersIdList = new ArrayObject();
        $maxOrdersReq = 50;//max ordini in una richiesta
        $throttlingCount = 0;
        $maxReq = 4; //max richieste in un minuto
        while ($amazonOrderId = tep_db_fetch_array($amazonListOrders)) {
            $arrayOrdersIdList->append($amazonOrderId['amazonOrderId']);
        }
        $orderDivided=new RecursiveArrayIterator(array_chunk($arrayOrdersIdList->getArrayCopy(),$maxOrdersReq));
        for( $i=0; $i < $orderDivided->count(); $i++ ){
            if($throttlingCount==$maxReq){
                sleep(60);
                $throttlingCount=0;
            }
            $orderDivided->seek($i);
            //var_dump($orderDivided->getChildren()->getArrayCopy());
            $orderIds->setId($orderDivided->getChildren()->getArrayCopy());
            $request->setAmazonOrderId($orderIds);
            // object or array of parameters
            invokeGetOrder($service, $request, $sede_amazon['sede']);
            $throttlingCount++;
        }
    }
}
 ...
 */
function invokeGetOrder(MarketplaceWebServiceOrders_Interface $service, $request, $sede)
{
    try {
        ...
        if ($response->isSetGetOrderResult()) {
            ...
            if ($getOrderResult->isSetOrders()) {
                ...
                foreach ($orderList as $order) {
                    ...
                    if ($order->isSetOrderStatus() && $order->getOrderStatus() != 'Pending') {
                        echo("                        OrderStatus\n");
                        echo("                            " . $order->getOrderStatus() . "\n");
                        tep_db_perform(TABLE_AMAZON_ORDER_LIST, array('amazonOrderStatus' => $order->getOrderStatus()), "update", "amazonOrderId = '" . $order->getAmazonOrderId() . "' AND amazonSede = ".$sede);
                    }
