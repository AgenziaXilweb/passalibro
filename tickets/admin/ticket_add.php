...
if (isset($_POST["Add"])) {
	...
	$arraysize = count($files["name"])-1;
		...
		if (empty($form_error)) {
			...
			mysqli_query($db, "INSERT INTO $mysql_ticket (User, User_Email, Cat_ID, Level_ID, Type, Subject, Message, Files, Status, Owner, Date_Added, Date_Updated $custom_sql_fields) VALUES ('$u', '$u_email', '$cat', '$level', 'Web', '$subject', '$message', '$files_for_sql', 'Open', $assignment, '$now', '$now' $custom_sql_vals)") or die(mysql_error());  
			...
			header("Location: ticket_add_post.php?ue=" . urlencode($u_email) . "&tid=" . urlencode($lastid));
...
<div id="body_filter" class="body_filter">
	...
	Fill out the form to raise a ticket.
...
<div id="body_page">
    ...
    while ($cf = mysqli_fetch_array($sel_custom_fields)) {
		...
		switch ($cf["Field_Type"]) {
			...
			echo "<input name=\"custom[".$cf["Field_Name"]."]\" type=\"text\" maxlength=\"".$cf["Field_MaxLen"]."\" value=\"".$custom_set_val[$cf["Field_Name"]]."\" placeholder=\"".$fieldname."\" /></p>";
			?>
			...
			case "Textbox":
			echo "<label for=\"".$custom_set_val[$cf["Field_Name"]]."\">".$fieldname."</label>";
			echo "<textarea name=\"custom[".$cf["Field_Name"]."]\" maxlength=\"".$cf["Field_MaxLen"]."\" placeholder=\"".$fieldname."\">".$custom_set_val[$cf["Field_Name"]]."</textarea></p>";
			?>
			<p><span class="form_desc error"><?php echo $form_error[$cf["Field_Name"]]; ?></span></p>
			<?php
			break;
