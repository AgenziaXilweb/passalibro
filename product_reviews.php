...
*/
  ...
  $product_info_query = tep_db_query("select p.products_used_quantity, p.products_id, pd.products_name, pd.products_description, p.products_model, p.products_quantity, p.products_image, pd.products_url, p.products_price, p.products_used_price, p.products_tax_class_id, p.products_date_added, p.products_date_available, p.manufacturers_id from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_status = '1' and p.products_id = '" . (int)$HTTP_GET_VARS['products_id'] . "' and pd.products_id = p.products_id and pd.language_id = '" . (int)$languages_id . "'");
...
<?php
  if (tep_not_null($product_info['products_image'])) {
    ...
    $nuovo = tep_draw_button('Compra Nuovo <b>'.$products_price.'</b>', 'cart', tep_href_link(basename($PHP_SELF), tep_get_all_get_params(array('action')) . 'action=buy_now'), 'primary',null,'nuovo');
    $usato = tep_draw_button('Compra Usato <b>'.$products_used_price.'</b>', 'cart', tep_href_link(basename($PHP_SELF), tep_get_all_get_params(array('action')) . 'action=buy_used_now'), 'primary',null,'usato');
    $prenotato = tep_draw_button('Prenota Nuovo <b>'.$products_price.'</b>', 'cart', tep_href_link(basename($PHP_SELF), tep_get_all_get_params(array('action')) . 'action=reserve_now'), 'primary',null,'prenotato');
?>
...
?>
  ...
  <div>
    <span style="float: left;"><?php echo sprintf(TEXT_REVIEW_DATE_ADDED, tep_date_long($reviews['date_added'])); ?></span><br />
