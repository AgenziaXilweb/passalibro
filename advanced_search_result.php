...
if(MODULE_WAREHOUSES_STATUS == 'True' && $dominio['active']==1){
    ...
    require(DIR_WS_WAREHOUSE . FILENAME_WAREHOUSE_SEARCH_RESULTS);    
...
// create column list
  ...
  $select_str = "select distinct " . $select_column_list . " m.manufacturers_id, p.products_ebay, p.products_id, pd.products_name, p.products_price, p.products_used_price, p.products_tax_class_id, pd.products_description, IF(s.status, s.specials_new_products_price, NULL) as specials_new_products_price, IF(s.status, s.specials_new_products_price, p.products_price) as final_price, IF(s.status, s.specials_new_products_price, p.products_used_price) as final_used_price, p.products_bookable ";
    ...
    for ($i=0, $n=sizeof($search_keywords); $i<$n; $i++ ) {
      switch ($search_keywords[$i]) {
        ...
        default:
          ...
          $where_str .= "(pd.products_name like '%" . tep_db_input($keyword) . "%' or p.products_model = '" . tep_db_input($keyword) . "'";
...
?>
  ...
  <div class="buttonSet">
    <?php echo tep_draw_other_button(IMAGE_BUTTON_BACK, 'triangle-1-w', tep_href_link(FILENAME_ADVANCED_SEARCH, tep_get_all_get_params(array('sort', 'page')), 'NONSSL', true, false)); ?>
  </div>
</div>
