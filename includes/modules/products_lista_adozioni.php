...
*/
  ...
  $prodotto = TABLE_PRODUCTS . ".products_id";
  $listing_ado = "select " . TABLE_PRODUCTS . ".products_id, " .
    TABLE_PRODUCTS_DESCRIPTION . ".products_name, " . TABLE_PRODUCTS .
    ".products_image, " . TABLE_PRODUCTS . ".products_price, " . TABLE_PRODUCTS .
    ".products_tax_class_id, " . TABLE_PRODUCTS . ".products_date_added from " .
    TABLE_PRODUCTS . " inner join " . ADOZSEDI . " on " . TABLE_PRODUCTS .
    ".cod_chiave = " . ADOZSEDI . ".cod_chiave inner join " .
    TABLE_PRODUCTS_DESCRIPTION . " on " . TABLE_PRODUCTS . ".products_id = " .
    TABLE_PRODUCTS_DESCRIPTION . ".products_id where " . ADOZSEDI . ".sede = " . $_REQUEST['sede'] .
    " and " . ADOZSEDI . ".cod_scuola = " . $_REQUEST['scuola'] . " and " . ADOZSEDI .
    ".classe = " . $_REQUEST['classe'] . " and " . ADOZSEDI . ".sezione = '" . $_REQUEST['sezione'] .
    "' and " . ADOZSEDI . ".anno = '2011'";
    
  $listing_split = new splitPageResults($listing_ado, MAX_DISPLAY_SEARCH_RESULTS, $prodotto);
  ...
  for ($col=0, $n=sizeof($column_list); $col<$n; $col++) {
    ...
    switch ($column_list[$col]) {
      ...
      case 'PRODUCT_LIST_PRICE':
        ...
        $lc_text = TABLE_HEADING_PRICE_USED;
        $lc_align = 'right';
        break;
...
####################################################################
    ...
    if ( ($column_list[$col] != 'PRODUCT_LIST_BUY_NOW') && ($column_list[$col] != 'PRODUCT_LIST_IMAGE') ) {
      empty($lc_text)?'':$lc_text = tep_create_sort_heading($HTTP_GET_VARS['sort'], $col+1, $lc_text);
    }
    ### MODIFICA PASSALIBRO: EVITO CHE VENGANO CREATE LE COLONNE DELLE QTA NUOVO/USATO ################################
    if (($column_list[$col]=='PRODUCT_LIST_QUANTITY_USED') || ($column_list[$col]=='PRODUCT_LIST_QUANTITY')) continue;#
    ###################################################################################################################
      ...
      for ($col=0, $n=sizeof($column_list); $col<$n; $col++) {
        switch ($column_list[$col]) {
          ...
          case 'PRODUCT_LIST_PRICE':
            ...
            case 'PRODUCT_LIST_PRICE_USED':
            if (tep_not_null($listing['specials_new_products_price'])) {
              $prod_list_contents .= '        <td align="right"><del>' .  $currencies->display_price($listing['products_used_price'], tep_get_tax_rate($listing['products_tax_class_id'])) . '</del>&nbsp;&nbsp;<span class="productSpecialPrice">' . $currencies->display_price($listing['specials_new_products_price'], tep_get_tax_rate($listing['products_tax_class_id'])) . '</span></td>';
            } else {
              $prod_list_contents .= '        <td align="right">' . $currencies->display_price($listing['products_used_price'], tep_get_tax_rate($listing['products_tax_class_id'])) . '</td>';
            }
            break;
...
eof stock announcement */
          ...
          case 'PRODUCT_LIST_BUY_NOW':
            if(($listing['products_quantity']>0) && ($listing['products_used_quantity']>0)){
                ...
                $prod_list_contents .= ' <td align="center">'. tep_draw_button(IMAGE_BUTTON_BUY_NOW, 'cart', tep_href_link(basename($PHP_SELF), tep_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $listing['products_id'])) .'<br>'.
                tep_draw_button(IMAGE_BUTTON_BUY_NOW_USED, 'cart', tep_href_link(basename($PHP_SELF), tep_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $listing['products_id'])). '</td>';
            
            }elseif(($listing['products_used_quantity']>0)&&($listing['products_quantity']<=0)){
                
                $prod_list_contents .= ' <td align="center">'. tep_draw_button(IMAGE_BUTTON_RESERVE_NOW, 'cart', tep_href_link(basename($PHP_SELF), tep_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $listing['products_id'])) .'<br>'.
                tep_draw_button(IMAGE_BUTTON_BUY_NOW_USED, 'cart', tep_href_link(basename($PHP_SELF), tep_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $listing['products_id'])). '</td>';                
            ...
            $prod_list_contents .='<td align="center">'.tep_draw_button(IMAGE_BUTTON_RESERVE_NOW, 'cart', tep_href_link(basename($PHP_SELF), tep_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $listing['products_id'])). '</td>';
            break;
            }
        }
      }
