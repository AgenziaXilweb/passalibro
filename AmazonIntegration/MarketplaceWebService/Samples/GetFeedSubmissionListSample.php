...
$feedId_query = tep_db_query("SELECT distinct(amazon_feedSubmissionId) as feedSubmissionId
                                FROM ".TABLE_AMAZON_FEEDS."
                               WHERE     amazon_feedProcessingStatus IS NOT NULL
                                AND amazon_sede=".$argv[1]."
...
while ($feedId = tep_db_fetch_array($feedId_query)) {
    ...
    $request->setFeedSubmissionIdList($idList->withId($feedId['feedSubmissionId']));
    invokeGetFeedSubmissionList($service, $request, $feedId['feedSubmissionId']);
    ...
    $ackFeedSubmissionId)
{
    try {
        ...
        if ($response->isSetGetFeedSubmissionListResult()) {
            ...
            $feedSubmissionInfoList = $getFeedSubmissionListResult->
            ...
            foreach ($feedSubmissionInfoList as $feedSubmissionInfo) {
               ...
               tep_db_perform(TABLE_AMAZON_FEEDS, $sql_data_array, 'update',
                      "amazon_feedSubmissionId = " . (int)$ackFeedSubmissionId);
