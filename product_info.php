...
*/
 ...
 if($_SERVER['HTTP_HOST']==$miohostname && $dominio['active']==1){
    ...
    require(DIR_WS_WAREHOUSE . FILENAME_WAREHOUSE_PRODUCT_INFO);
...
<?php
    ...
    $product_info_query = tep_db_query("select p.products_ebay, p.products_used_quantity, p.products_id, pd.products_name, pd.products_description, p.products_model, p.products_quantity, p.products_image, pd.products_url, p.products_price, p.products_used_price, p.products_tax_class_id, p.products_date_added, p.products_date_available, p.manufacturers_id, p.products_bookable from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_status = '1' and p.products_id = '" . (int)$HTTP_GET_VARS['products_id'] . "' and pd.products_id = p.products_id and pd.language_id = '" . (int)$languages_id . "'");
...
<div class="buttonSet">
    ...
    $prenotato = $product_info['products_bookable']?tep_draw_button('Prenota Nuovo <b>'.$products_price.'</b>', 'cart', tep_href_link(FILENAME_PRODUCT_INFO, urldecode(tep_get_all_get_params()) . 'action=reserve_now'), 'primary',null,'prenotato'):'Non Prenotabile';
   ...
   if($product_info['products_quantity'] == 0) {
      ...
      $checkebay=$product_info['products_ebay'] == 0?$prenotato:'';
