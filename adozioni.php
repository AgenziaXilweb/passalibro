...
if ($_REQUEST['azione'] == '') {
       ...
       if(MODULE_WAREHOUSES_STATUS == 'True' && $dominio['active']==1){
        ...
        $_REQUEST['sede'] = $dominio['sede'];

        $_REQUEST['nomesede'] = $dominio['citta'];
        
        goto citta;  
        
        }
    
    $link = "SELECT sede, citta FROM ". TABLE_SEDI;
    
    if(MODULE_WAREHOUSES_STATUS == 'True' && $dominio['active']==0){
        
    $link .=" WHERE sede IN(".$dominio['includi'].")"; 
    
    }
    
    $query_sedi = mysql_query($link);

    # INIZIO LISTA SEDI

    while ($libreria = mysql_fetch_array($query_sedi)) {
        
        echo "<div id='bgsedi'><div class='sedi'><a href=" . FILENAME_ADOZIONI . "?nomesede=" .
            strtoupper(str_replace(' ', '%20', $libreria['citta'])) . "&sede=" . $libreria['sede'] .
            "&azione=citta >" . $libreria['citta'] . "</a></div></div>";

    }
    
        echo "<div id='bgsedi'><div class='sedi'><a href='" . FILENAME_ADOZIONI . "?azione=mialista'>Compila la tua lista</a></div></div>";
...
<input class="mybutton" type="button" onclick="this.form.submit();" value="Cerca" />
    ...
    while ($istituto = mysql_fetch_array($query_paesi)) {
        ...
        if ($i++ % 3 == 0)
            echo "<tr>\n";
        echo "<td valign='top' width='33%'><a href=" . FILENAME_ADOZIONI .
            "?azione=scuole&sede=" . $istituto['sede'] . "&citta=" . str_replace(' ',
            '%20', $istituto['citta']) . ">" . strtoupper($istituto['citta']) . "</a></td>";
        if ($i % 3 == 0)
            echo "</tr>\n";
    }
...
$query_scuole = mysql_query("SELECT " . SCUOLE_SEDI . ".cod_scuola, 
    ...
    while ($istituto = mysql_fetch_array($query_scuole)) {
        ...
        echo "<td valign='top' width='33%'><font color='red''><b>" . strtoupper($istituto['citta']) .
            "</b></font><br><a href=" . FILENAME_ADOZIONI . "?azione=classisedi" .
            "&sede=" . $_REQUEST['sede'] . "&scuola=" . $istituto['cod_scuola'] .
            "&nomescuola=" . str_replace(' ', '%20', $istituto['istituto']) . str_replace(' ',
...
$query_classi = mysql_query("SELECT * 
    ...
    while ($istituto = mysql_fetch_array($query_classi)) {
        ...
        echo "<td valign='top' width='33%'><a href=" . FILENAME_ADOZIONI_PRODOTTI .
            "?sede=" . $_REQUEST['sede'] . "&classe=" . $istituto['classe'] . "&sezione=" .
...
if ($_REQUEST['azione'] == 'mialista') {
    
echo '<center><h1>Cerca nelle adozioni il tuo testo</h1>
<div id="mylist" class="mysearch">
<form method="post" action="'. FILENAME_ADOZIONI_PRODOTTI . '?sede='.$_REQUEST['sede'].'&azione=search">
<input class="myinputbox" value="WORK IN PROGRESS..." type="text" name="richiesta" disabled/>
<input class="mybutton" type="button" onclick="this.form.submit();" value="Cerca" disabled/>
</form></div></center>';
