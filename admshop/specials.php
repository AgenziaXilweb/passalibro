...
$cercalibro = isset($_GET['name'])?'<input size="100%" type="text" name="name" value="'.$_GET['name'].'" readonly="readonly"/><input type="hidden" name="products_id" value="'.$_GET['products_id'].'"/>':'';
?>
      ...
      <tr><form name="new_special" <?php echo 'action="' . tep_href_link(FILENAME_SPECIALS, tep_get_all_get_params(array('action', 'info', 'sID')) . 'action=' . $form_action, 'NONSSL') . '"'; ?> method="post"><?php if ($form_action == 'update') echo tep_draw_hidden_field('specials_id', $HTTP_GET_VARS['sID']); ?>
        <td><br /><table border="0" cellspacing="0" cellpadding="2">
          <tr>
            ...
            <td class="main"><?php echo (isset($sInfo->products_name)) ? $sInfo->products_name . ' <small>(' . $currencies->format($sInfo->products_price) . ')</small>' : $cercalibro, tep_draw_button('CERCA PER ISBN O PER TITOLO', 'disk', tep_href_link('specials_search.php')); echo tep_draw_hidden_field('products_price', (isset($sInfo->products_price) ? $sInfo->products_price : ''));?></td>
          ...
          <tr>
            ...
            <td class="main"><?php echo tep_draw_input_field('specials_price', (isset($sInfo->specials_new_products_price) ? $sInfo->specials_new_products_price : '')), ' Il prezzo ufficale nel catalogo è ', number_format($_GET['prezzo'],2,',','.').' euro'; ?></td>
          </tr>
          <tr>
