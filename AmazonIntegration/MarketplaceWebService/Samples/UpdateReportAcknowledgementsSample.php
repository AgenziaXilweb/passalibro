...
$sediPassalibro = tep_db_query("SELECT distinct(report.sede) as sede, account.merchantID
                                FROM " . TABLE_AMAZON_REPORTS_LIST. " report JOIN ".TABLE_ACCOUNT_EXTERNAL." account USING(sede) WHERE account.piattaforma='amazon' AND report.reportsAcknowledged = 0");
while($sede = tep_db_fetch_array($sediPassalibro)){
    ...
    while ($report = tep_db_fetch_array($reportsList_query)) {        
        ...
        $request->setReportIdList($idList->withId($report['reportsId']));
