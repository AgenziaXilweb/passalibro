...
require('warehouse_split_result_pages.php');

$listing_split = new WareHouse_splitPageResults($listing_sql, MAX_DISPLAY_SEARCH_RESULTS, 'p.products_id');
...
<?php if (($listing_split->number_of_rows > 0) && ((PREV_NEXT_BAR_LOCATION == '1') || (PREV_NEXT_BAR_LOCATION == '3'))) { ?>
    ...
    <div id="pg"><span style="float: right;"><?php echo $listing_split->display_links(MAX_DISPLAY_PAGE_LINKS, tep_get_all_get_params(array(
        'page',
        'info',
        'x',
        'y'))); ?>
...
$prod_list_contents = '<div class="mediagroove"><div>' .
    '    <table><caption>' . MODULE_WAREHOUSE_TEXT_TITLE . '</caption>' .
    '      <thead><tr>';
        ...
        for ($col = 0, $n = sizeof($column_list); $col < $n; $col++) {
            ...
            switch ($column_list[$col]) {
                ...
                case 'PRODUCT_LIST_IMAGE':
                    ...
                    #MODIFICATO PER PASSALIBRO
                case 'PRODUCT_LIST_BUY_NOW':
      
                    
                    if (($listing['nuovo'] > 0) && ($listing['usato'] > 0)) {
                    ...
                    } elseif (($listing['products_ebay'] == false)&&($listing['usato'] > 0) && ($listing['nuovo'] <= 0)) {
                        
                        $prod_list_contents .= ' <td width="15%" align="center">' . tep_draw_button(IMAGE_BUTTON_RESERVE_NOW . '<br><b>&euro;' . str_replace('.',',',money_format('%.2n', $listing['products_price'])) . '</b>'.TEXT_SHIPPING_DELIVERY_TIME_RESERVED,
                            'cart', tep_href_link(basename($PHP_SELF), tep_get_all_get_params(array('action')) .
                            'action=reserve_now&products_id=' . $listing['products_id']),null ,null, 'prenotato') . '<br>' .
                            tep_draw_button(IMAGE_BUTTON_BUY_NOW_USED . '<br><b>&euro;' . str_replace('.',',',money_format('%.2n', $listing['products_used_price'])) . '</b>'.TEXT_SHIPPING_DELIVERY_TIME_USED, 'cart', tep_href_link(basename($PHP_SELF), tep_get_all_get_params(array('action')) .
                            'action=buy_used_now&products_id=' . $listing['products_id']),null ,null, 'usato') . '</td>';

                    } elseif (($listing['usato'] <= 0) && ($listing['nuovo'] > 0)) {
                        
                        $prod_list_contents .= '<td width="15%" align="center">' . tep_draw_button(IMAGE_BUTTON_BUY_NOW . '<br><b>&euro;' . str_replace('.',',',money_format('%.2n', $listing['products_price'])) . '</b>'.TEXT_SHIPPING_DELIVERY_TIME_NEW,
                            'cart', tep_href_link(basename($PHP_SELF), tep_get_all_get_params(array('action')) .
                            'action=buy_now&products_id=' . $listing['products_id']),null ,null, 'nuovo') . '</td>';
                            
                    } elseif (($listing['products_ebay'] == false)) {
                            
                        $prod_list_contents .= '<td width="15%" align="center">' . tep_draw_button(IMAGE_BUTTON_RESERVE_NOW . '<br><b>&euro;' . str_replace('.',',',money_format('%.2n', $listing['products_price'])) . '</b>'.TEXT_SHIPPING_DELIVERY_TIME_RESERVED,
                            'cart', tep_href_link(basename($PHP_SELF), tep_get_all_get_params(array('action')) .
                            'action=reserve_now&products_id=' . $listing['products_id']),null ,null, 'prenotato') . '</td>';

                    }elseif (($listing['products_ebay'] == true)&& ($listing['nuovo'] > 0)) {
                        
                        $prod_list_contents .= '<td width="15%" align="center">' . tep_draw_button(IMAGE_BUTTON_BUY_NOW . '<br><b>&euro;' . str_replace('.',',',money_format('%.2n', $listing['products_price'])) . '</b>'.TEXT_SHIPPING_DELIVERY_TIME_NEW,
                            'cart', tep_href_link(basename($PHP_SELF), tep_get_all_get_params(array('action')) .
                            'action=buy_now&products_id=' . $listing['products_id']),null ,null, 'nuovo') . '</td>';

                    }elseif (($listing['products_ebay'] == true)&& ($listing['usato'] > 0)) {
                        
                        $prod_list_contents .= '<td width="15%" align="center">' . tep_draw_button(IMAGE_BUTTON_BUY_NOW_USED . '<br><b>&euro;' . str_replace('.',',',money_format('%.2n', $listing['products_used_price'])) . '</b>'.TEXT_SHIPPING_DELIVERY_TIME_USED,
                            'cart', tep_href_link(basename($PHP_SELF), tep_get_all_get_params(array('action')) .
                            'action=buy_used_now&products_id=' . $listing['products_id']),null ,null, 'usato') . '</td>';
