 ...
 ***********************************************************************/
 ...
 $request->setSellerId('A36QOOGIRQGSK2');
 $request->setMarketplaceId('APJ6JRA9NG5V4');
 $asini= tep_db_query("select * from passalibro.catalogo c join passalibro.catalogo_sedi cs using(cod_chiave) where cs.sede=3 and cs.pubblicato_amazon=1 limit 10");
 while ($asin = tep_db_fetch_array($asini)) {
     $request->setQuery($asin['isbn13']);
