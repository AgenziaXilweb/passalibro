    ...
    switch ($action) {
      case 'insert':
        ...
        if (tep_db_num_rows($check_query) < 1) {
          tep_db_query("insert into " . TABLE_ADMINISTRATORS . " (user_name, user_password, sede) values ('" . tep_db_input($username) . "', '" . tep_db_input(tep_encrypt_password($password)) . "', '" . $_SESSION['admsede'] . "')");
...
<?php
  $admins_query = tep_db_query("select id, user_name from " . TABLE_ADMINISTRATORS . " where sede=".$_SESSION['admsede']." order by user_name");
