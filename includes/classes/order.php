...
*/
  ...
  class order {
    ...
    function query($order_id) {
      ...
      $index = 0;
      $orders_products_query = tep_db_query("select orders_products_id, products_id, products_name, products_model, products_price, products_used_price, products_tax, products_quantity, products_used_quantity, products_quantity_reserved, final_price from " . TABLE_ORDERS_PRODUCTS . " where orders_id = '" . (int)$order_id . "'");
      while ($orders_products = tep_db_fetch_array($orders_products_query)) {
        $this->products[$index] = array('qty' => $orders_products['products_quantity'],
                                        'used_qty' => $orders_products['products_used_quantity'],
                                        'reserved_qty' => $orders_products['products_quantity_reserved'],
	                                    'id' => $orders_products['products_id'],
                                        'name' => $orders_products['products_name'],
                                        'model' => $orders_products['products_model'],
                                        'tax' => $orders_products['products_tax'],
                                        'price' => $orders_products['products_price'],
                                        'used_price' => $orders_products['products_used_price'],
    ...
    function cart() {
      ...
      for ($i=0, $n=sizeof($products); $i<$n; $i++) {
        ...
        $this->products[$index] = array('qty' => $products[$i]['quantity'],
                                        'used_qty' => $products[$i]['used_quantity'],
                                        'reserved_qty' => $products[$i]['reserved_quantity'],
                                        'name' => $products[$i]['name'],
                                        'model' => $products[$i]['model'],
                                        'ebay' => $products[$i]['ebay'],
                                        'tax' => tep_get_tax_rate($products[$i]['tax_class_id'], $tax_address['entry_country_id'], $tax_address['entry_zone_id']),
                                        'tax_description' => tep_get_tax_description($products[$i]['tax_class_id'], $tax_address['entry_country_id'], $tax_address['entry_zone_id']),
                                        'price' => $products[$i]['price'],
                                        'used_price' => $products[$i]['used_price'],
                                        'final_price' => $products[$i]['price'] + $cart->attributes_price($products[$i]['id']),
                                        'final_used_price' => $products[$i]['used_price'] + $cart->attributes_price($products[$i]['id']),
                                        'data_school'=>$products[$i]['data_school'],
                                        'sede_impegno_nuovo' => $products[$i]['sede_impegno_nuovo'],
                                        'sede_impegno_usato' => $products[$i]['sede_impegno_usato'],
        ...
        $shown_price += $currencies->calculate_price($this->products[$index]['final_used_price'], $this->products[$index]['tax'], $this->products[$index]['used_qty']);
        $shown_price += $currencies->calculate_price($this->products[$index]['final_price'], $this->products[$index]['tax'], $this->products[$index]['reserved_qty']);
