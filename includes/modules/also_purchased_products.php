  ...
  if (isset($HTTP_GET_VARS['products_id'])) {
    ...
    if ($num_products_ordered >= MIN_DISPLAY_ALSO_PURCHASED) {
      ...
      while ($orders = tep_db_fetch_array($orders_query)) {
        ...
        $ebay_image=$orders['products_ebay']==1?$orders['products_image']:DIR_WS_IMAGES . 'nopic.png';
        $pictures = file_exists(DIR_WS_ISBN . $orders['products_image'])? DIR_WS_ISBN . $orders['products_image']: $ebay_image;
        
        $also_pur_prods_content .= '<td width="33%" valign="top" align="center"><a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $orders['products_id']) . '">' . tep_image($pictures, $orders['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a><br /><a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $orders['products_id']) . '">' . $orders['products_name'] . '</a></td>';
